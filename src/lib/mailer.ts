...
export async function sendEnquiryMail({ name, email, phone, address, subject, message, products }: EnquiryMailOptions) {
  ...
  const total = isCart ? products!.reduce((s, p) => s + (p.price ?? 0) * p.quantity, 0) : 0;
...
<body style="margin:0;padding:0;background:#fdf8f3;font-family:Arial,sans-serif;">
  <div style="max-width:600px;margin:32px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
    <div style="background:linear-gradient(135deg,#8B1A1A,#c0392b);padding:28px 32px;">
      <h1 style="margin:0;color:#fff;font-size:20px;font-weight:700;">${isCart ? "New Cart Enquiry" : "New Enquiry"}</h1>
    ...
    <div style="padding:24px 32px;border-bottom:1px solid #f0e8dc;">
      ...
      <table style="width:100%;border-collapse:collapse;font-size:14px;">
        ...
        ${total > 0 ? `<tfoot><tr style="background:#fdf8f3;">
